...
class Users {
  ...
  updateOne(username, body) {
    ...
    const usernameExist = items.findIndex((item) => item.username == body.username);
    ...
    serialize(this.jsonDbPath, items);
    const objet = {
      username:items[foundIndex].username,
    }
    return objet;
  }
  async updatePassword(username,body){
    const items = parse(this.jsonDbPath, this.defaultItems);
    const foundIndex = items.findIndex((item) => item.username == username);
    if (foundIndex < 0) return;
    // checked hash of passwords
    const match = await bcrypt.compare(body.oldPassword,items[foundIndex].password);
    if (!match) return;
    const hashedPassword = await bcrypt.hash(body.nPassword, saltRounds);
    const newItem = {
      username:username,
      password:hashedPassword,
      maxScore:items[foundIndex].maxScore,
    }
